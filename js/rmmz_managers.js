...
DataManager.saveGame = function(savefileId) {
    const contents = this.makeSaveContents();  // Contents of the save file
    const saveName = this.makeSavename(savefileId);  // Name of the save file
    return StorageManager.saveObject(saveName, contents).then(() => {
        this._globalInfo[savefileId] = this.makeSavefileInfo();
        this._globalInfo[savefileId].gameID = "LoneStars1.0";  // Adding unique identifier
        this.saveGlobalInfo();
        return 0;
    });
};
...
DataManager.loadGame = function(savefileId) {
    ...
    return StorageManager.loadObject(saveName).then(contents => {
        ...
        if (this._globalInfo[savefileId].gameID !== "LoneStars1.0") {  // Check your unique identifier
            console.error("Save file does not match the current game version.");
            return false;
        }
        return true;
    });
